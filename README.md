...
All calculations run locally in the userÕs browser. No input data is sent to an external server.
...
Methodology
1. Sainte-Lagu‘
Sainte-Lagu‘ is a divisor method that allocates seats by dividing each partyÕs vote total by a sequence of odd-numbered divisors.
Divisors = 1, 3, 5, 7, 9, ...
The highest quotients receive seats until all seats are allocated.
Sainte-Lagu‘ is often considered more proportional than DÕHondt because it is less favorable to larger parties.

2. Modified Sainte-Lagu‘
Modified Sainte-Lagu‘ uses the same divisor sequence as Sainte-Lagu‘, except that the first divisor is changed from 1 to 1.4.
...
3. DÕHondt
DÕHondt is a divisor method using consecutive integer divisors.
Divisors = 1, 2, 3, 4, 5, ...
Compared with Sainte-Lagu‘, DÕHondt tends to produce more favorable outcomes for larger parties.
...
Electoral Researcher á Policy Analyst
LinkedIn:
